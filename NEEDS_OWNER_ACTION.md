...
* `docs/ARCHITECTURE.md` — every design decision and its rationale, including the ones where
  ...
  dedup reasoning).
* 45 tests that execute the real compiled artifact, all asserting on observable onchain
  state.
...
| Rust test suite | **45 passing, exit 0.** Executes the real compiled artifact. |
