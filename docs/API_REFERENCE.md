...
* **No TypeScript type for the program's IDL.** The SDK hand-encodes the instruction data
  ...
  SDK imports it.
* **No `createClaimInstruction` helper.** The error message thrown by `getSubtleCrypto`
  tells callers to "pass them to the low-level API (`createClaimInstruction` /
  `deriveReceiptAddress`)". `deriveReceiptAddress` exists; `createClaimInstruction` does
  not. The low-level path that does exist is `encodeClaimData` plus a hand-built
  `Instruction`, or `prepareIntent` with a precomputed 32-byte `intent`.
