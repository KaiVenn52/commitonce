...
## 4. Rust test suite — 60 passing, exit code 0
...
REAL_EXIT=0   TOTAL_PASSED=60   TOTAL_FAILED=0
```
...
| Deployment matches the source | **The deployed bytecode is the committed artifact.** `scripts/verify-deployment.sh` dumps both programs from devnet and compares them against `target/deploy/*.so`: the first 148,640 bytes of `commit_once` and the first 158,432 bytes of `demo_counter` are **byte-identical**, with the loader having appended 15,072 and 10,808 bytes of zero padding respectively. It also checks the slots and the upgrade authority. **A whole-file hash comparison would have reported a mismatch** for both, which is what the first investigation did before the difference was understood. Evidence: [`submission/evidence/devnet-deployment-matches-artifact.log`](submission/evidence/devnet-deployment-matches-artifact.log). |
...
| CPI from a non-Anchor program | **Closed.** `programs-native/native-guard` is a program with **no Anchor dependency at all** that guards its own action with CommitOnce, and `tests/native_cpi.rs` proves it: five tests, including that the action runs once, that a rebuilt retry is blocked, and that the hand-written account order is load-bearing. It builds the `claim` instruction's 144 bytes itself — discriminator then Borsh body — orders the account metas the way the callee declares them, and calls `invoke`, because it has no `CpiContext` to do any of that. This was previously recorded as *"what is not tested is a CPI into `claim` from a non-Anchor onchain program; that needs a second program, and none has been written."* It has been written. |
| Non-Anchor clients | **Verified, and the limitation was previously overstated.** The SDK has **zero runtime dependencies**, imports nothing from Anchor's JavaScript library — every mention of "Anchor" in `packages/sdk/src/` is a comment explaining which discriminator is being reproduced — and hand-encodes the instruction discriminator, the Borsh body and the base64 event decoding. It has been executed against the deployed program on devnet repeatedly. The CPI half is now tested too: **Closed.** `programs-native/native-guard` is a program with **no Anchor dependency at all** that guards its own action with CommitOnce, and `tests/native_cpi.rs` proves it: five tests, including that the action runs once, that a rebuilt retry is blocked, and that the hand-written account order is load-bearing. It builds the `claim` instruction's 144 bytes itself — discriminator then Borsh body — orders the account metas the way the callee declares them, and calls `invoke`, because it has no `CpiContext` to do any of that. This was previously recorded as *"what is not tested is a CPI into `claim` from a non-Anchor onchain program; that needs a second program, and none has been written."* It has been written. |
...
**seventeen steps** — then prints a PASS/FAIL summary with the true exit code. It fails loudly
...
Last full run: **`RESULT: PASS (17 steps ran and passed)`**, exit code 0.
