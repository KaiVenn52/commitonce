...
authority (a Squads vault can be the authority: a PDA becomes a `Signer` only through
`invoke_signed`, and both that and the CPI are tested in `tests/cpi.rs` — but per-member keys
cannot share one receipt unless they share a vault);
...
pnpm --filter @commitonce/solana test                      # 79 tests
