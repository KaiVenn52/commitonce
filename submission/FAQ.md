...
| Transaction v1 messages | **Not tested.** v1 is active on mainnet, devnet and testnet, per Solana's versioned-transactions documentation. It raises the size limit to 4,096 bytes, moves resource limits into the message itself, and **removes address lookup tables** — a v1 transaction inlines up to 64 addresses instead. Whether v1 changes message-hash deduplication is an expectation, not a measurement. |
...
| CPI into `claim` from another program | **Tested** in `tests/cpi.rs`, seven tests across two instructions: `increment_guarded` (transaction signer as authority) and `increment_guarded_by_vault` (**PDA** authority via `invoke_signed`). The invariant holds through both, and the guard program account cannot be substituted. |
| Genuine durable-nonce transactions | **Tested against devnet.** The harness cannot express one, so `tests/security.rs` injects the marker; `apps/demo/nonce-policy.ts` builds real ones with a genuine nonce account and confirms finite retention is refused (6002) and `permanent` is allowed. Raw output: [`evidence/devnet-nonce-policy-run.log`](evidence/devnet-nonce-policy-run.log). |
...
| **Composability with other programs** | **Executed, not just reasoned about.** Three of the four examples have run against devnet against the deployed program: `sol-transfer` (System Program), `spl-transfer` (SPL Token + Associated Token in one atomic transaction), `custom-program` (an arbitrary Anchor program, where the retry carried a *different* instruction set from the first attempt and was still blocked). Raw output in [`evidence/`](evidence/). The Jupiter-swap example has been **composed against a real Jupiter transaction** fetched from Jupiter's live API, verified in dry-run mode, but has **not** executed end to end — Jupiter's aggregator is mainnet-only and CommitOnce is devnet-only. |
