...
| CPI into `claim` from another program | **Tested** in `tests/cpi.rs`. `demo-counter` calls `claim` itself via `increment_guarded`; the invariant holds through the CPI, and the guard program account is constrained so it cannot be substituted. |
